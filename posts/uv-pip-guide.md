> **核心逻辑**
> uv 分为两种使用模式：
> + 「项目模式」：默认用 `pyproject.toml` 管理依赖（适合需要打包、共享的项目）；
> + 「无配置模式」：用 `uv pip` 命令，完全兼容 pip 语法，不用任何配置文件（适合机器学习、快速实验类项目）。
>
...
> **注意**
> `uv` 创建的虚拟环境 **不包含 Python 解释器本身**，只记录了 **绝对路径** 指向原来的解释器
>
> + 如果使用`uv venv`，只要不带 --python <版本>，uv 就会按 `PATH` 顺序找系统里已安装的任何 Python（官方安装包、Miniconda、MS Store…都可），找到第一个就拿来用，在 `pyvenv.cfg` 里写下它的路径。这里不会触发下载，也不会去 uv 自己的“工具箱”里翻。
> + 如果使用`uv venv --python 3.12`，uv 会先看自己“工具箱”里有没有 **通过 `uv python install` 下载的 3.12**（`~/.local/share/uv/python/python-3.12.*` 或 Windows 的 `%LOCALAPPDATA%\uv\data\python\...`），如果找到了，就直接把虚拟环境指向这一份 **便携版 3.12**；找不到就自动去下载，再指向它。
> + 所以本质上`uv`在`.venv`里管理的只有当前项目的依赖包，而`Python`全部安装在`UV`统一管理的 Python 路径（`%LOCALAPPDATA%\uv\data\python`）中，`uv`会自动生成快捷方式来指向你想要的版本的`Python`
...
> + 一堆第三方包
> + 启动脚本（python → 指向某绝对路径）
> + 配置（pyvenv.cfg 里写明了“home = 原解释器路径”）
...
> **警告**
> **所以说，即使使用 `uv venv --python 3.12` 和 `uv python install 3.12`，`uv` 实际上也是先把 `Python` 下到自己的目录，然后再在 `.venv` 里写一个指向这个 `Python` 的快捷方式。**
>
